...
def Algr1():#First algorithm: main 
    ...
    while change == 1 and '?' in SudokuToSolve:
        ...
        while CheckingCell < 81:
            ...
            if SudokuToSolve[CheckingCell] == '?':
                CellPossibilities = len(set1to9 - set(SudokuToSolveRows[CheckingRow]) - set(SudokuToSolveColumns[CheckingColumn]) - set(SudokuToSolveSquares[CheckingSquare]))
        ...
        while CheckingRow2 < 9: #which row(
            ...
            while CheckingElementRequirement < len(RowRequirements):#Which requirement(
                ...
                if AmountOfPlacesRows == 1:
                    ...
                    SudokuToSolve = SudokuToSolveCopy
                    change2 = 1
                if AmountOfPlacesRows == 0:
                    print('RowsERROR')
                    breakpoint()
                CheckingElementRequirement = CheckingElementRequirement + 1
            CheckingRow2 = CheckingRow2 + 1
#NOW FOR Algorithm 2.2; columns			
    CheckingColumn2 = 0
    while CheckingColumn2 < 9: #which Column(
        SudokuToSolveCopy = SudokuToSolve
        SudokuToSolveRowsCopy = SudokuToSolveRows
        SudokuToSolveColumnsCopy = SudokuToSolveColumns
        SudokuToSolveSquaresCopy = SudokuToSolveSquares
        ColumnRequirements = list(set1to9 - set(SudokuToSolveColumns[CheckingColumn2]))
        ColumnRequirements = sorted(ColumnRequirements)
        CheckingElementRequirementColumns = 0
        while CheckingElementRequirementColumns < len(ColumnRequirements):#Which requirement(
            TryCellColumns = 0
            AmountOfPlacesColumns = 0
            SaveElementColumns = 0
            SudokuToSolveCopy = SudokuToSolve
            SudokuToSolveRowsCopy = SudokuToSolveRows
            SudokuToSolveColumnsCopy = SudokuToSolveColumns
            SudokuToSolveSquaresCopy = SudokuToSolveSquares
            ColumnRequirements = list(set1to9 - set(SudokuToSolveColumns[CheckingColumn2]))
            ColumnRequirements = sorted(ColumnRequirements)
##            ColumnRequirements = ColumnRequirements + ['-1','-1','-1','-1']
            while TryCellColumns < 9: #Which trying cell(
                SudokuToSolveCopy = SudokuToSolve
                SudokuToSolveRowsCopy = SudokuToSolveRows
                SudokuToSolveColumnsCopy = SudokuToSolveColumns
                SudokuToSolveSquaresCopy = SudokuToSolveSquares
                if SudokuToSolveColumns[CheckingColumn2][TryCellColumns] == '?':
                    if ColumnRequirements[CheckingElementRequirementColumns] in list(set1to9 - set(SudokuToSolveRows[TryCellColumns]) - set(SudokuToSolveSquares[TryCellColumns//3 + (CheckingColumn2//3)*3])):
                        AmountOfPlacesColumns = AmountOfPlacesColumns + 1
                        SaveCellColumns = TryCellColumns
                        SaveElementColumns = CheckingElementRequirementColumns#)
#                print('rep',CheckingColumn2,TryCellColumns)
                TryCellColumns = TryCellColumns + 1
            if AmountOfPlacesColumns == 1:
                SudokuToSolveRowsCopy[CheckingColumn2][SaveCellColumns] = ColumnRequirements[SaveElementColumns]
                SudokuToSolveCopy[CheckingColumn2*9 + SaveCellColumns] = SudokuToSolveRowsCopy[CheckingColumn2][SaveCellColumns]
                SudokuToSolveColumnsCopy[SaveCellColumns][CheckingColumn2] = ColumnRequirements[SaveElementColumns]
                #breakpoint()
                WeirdGlitchHelper = ColumnRequirements[SaveElementColumns]# never mind, figured out glitch, but code stays :D
                SudokuToSolveSquaresCopy[SaveCellColumns//3 + (CheckingColumn2//3)*3][(((CheckingColumn2*9 + SaveCellColumns)%27)//9)*3+(((CheckingColumn2*9 + SaveCellColumns)%27)%3)] = WeirdGlitchHelper
                SudokuToSolveSquares = SudokuToSolveSquaresCopy
                SudokuToSolveColumns = SudokuToSolveColumnsCopy
                SudokuToSolveRows = SudokuToSolveRowsCopy
                SudokuToSolve = SudokuToSolveCopy
                change2 = 1
            if AmountOfPlacesColumns == 0:
                print('ColumnsERROR')
                breakpoint()
            CheckingElementRequirementColumns = CheckingElementRequirementColumns + 1
        CheckingColumn2 = CheckingColumn2 + 1
